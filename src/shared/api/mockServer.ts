...
type AppRegistry = Registry<
	...
	// biome-ignore lint/complexity/noBannedTypes: Mirage's Registry type expects this empty factories map shape.
	{}
>;
...
createServer({
	...
	routes() {
		...
		this.get("/vans/:id", (schema: AppSchema, request) => {
			return schema.find("vans", request.params.id);
		});
		...
		this.get("/host/vans/:id", (schema: AppSchema, request) => {
			return schema.findBy("vans", {
				id: request.params.id,
				hostId: "123",
			});
		});

		this.post("/sign-in", (schema: AppSchema, request) => {
